...
async function main() {
    ...
    // Fungsi untuk mengacak urutan array (Fisher-Yates)
    ...
    for (let epoch = 1; epoch <= epochs; epoch++) {
        ...
        // Acak dataset setiap awal epoch untuk mencegah catastrophic forgetting
        shuffleArray(datasetInputs);
        ...
        for (let i = 0; i < datasetInputs.length; i++) {
            ...
            // Linear Learning Rate Decay (Menurun perlahan seiring berjalannya iterasi)
            // Memastikan representasi yang dipelajari di akhir dataset tidak menghancurkan yang di awal
            const currentLR = learningRate * Math.max(0.01, (1 - (iterCount / datasetInputs.length)));

            // 4. Single-Shot Forward & BPTT (Dinamika Temporal terpusat di Sekuens BPTT)
            const result = model.forwardAndLearnLocal(inputs, B_emb, currentLR);
