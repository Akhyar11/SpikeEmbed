...
async function main() {
    ...
    // Gunakan konfigurasi standar (karena kita mengabaikan file konfigurasi bobot)
    const d_model = 64;
    ...
    // MEMUAT BOBOT HASIL TRAINING (Bi-Encoder)
    if (fs.existsSync('./models/spiking_model_weights.json')) {
        ...
        console.log("      -> SNN Siap! (Dimensi: 64, Bobot BERHASIL DIMUAT dari hasil pelatihan)");
    } else {
        console.log("      -> [PERINGATAN] SNN Siap! (File bobot tidak ditemukan, menggunakan RANDOM INITIALIZATION)");
    }
